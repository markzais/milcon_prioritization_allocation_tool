server <- function(input, output, session){
  ...
  template <- read.csv(paste(path, "/", "Input_Data_Template", ".csv", sep = ""),
                       as.is = T, header = T)
  ...
  obs2 <- observeEvent(input$load,{
    ...
    # Create a stronger stability matrix (1, 2, 4, 8, 16) ## JGD 20190220
    # stabilityMatrix <- matrix(1,m,p)
    # for (i in 1:m){
    #   if (previousProgram[i]==y){
    #     stabilityMatrix[i,1] = 16
    #     stabilityMatrix[i,2] = 8
    #     stabilityMatrix[i,3] = 4
    #     stabilityMatrix[i,4] = 2
    #     stabilityMatrix[i,5] = 1
    #   }
    #   if (previousProgram[i]==y+1){
    #     stabilityMatrix[i,1] = 8
    #     stabilityMatrix[i,2] = 16
    #     stabilityMatrix[i,3] = 8
    #     stabilityMatrix[i,4] = 4
    #     stabilityMatrix[i,5] = 2
    #   }
    #   if (previousProgram[i]==y+2){
    #     stabilityMatrix[i,1] = 4
    #     stabilityMatrix[i,2] = 8
    #     stabilityMatrix[i,3] = 16
    #     stabilityMatrix[i,4] = 8
    #     stabilityMatrix[i,5] = 4
    #   }
    #   if (previousProgram[i]==y+3){
    #     stabilityMatrix[i,1] = 2
    #     stabilityMatrix[i,2] = 4
    #     stabilityMatrix[i,3] = 8
    #     stabilityMatrix[i,4] = 16
    #     stabilityMatrix[i,5] = 8
    #   }
    #   if (previousProgram[i]==y+4){
    #     stabilityMatrix[i,1] = 1
    #     stabilityMatrix[i,2] = 2
    ...
    observeEvent(input$solve, {
      ...
      output$download <- downloadHandler(
        ...
        content = function(file) {
          write.csv(outputTable(), file, row.names = FALSE) # JGD 20190221 changed to row.names = FALSE
        }
      )
